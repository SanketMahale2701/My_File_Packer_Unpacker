import javax.swing.*;
import java.awt.*;
import java.awt.event.ActionEvent;   //447
import java.awt.event.ActionListener;
import java.io.FileInputStream;
public class MyPackFront extends Template implements ActionListener
{
    ...
    {  
      ...
      title = new JLabel("My Packing Portal");
      ...
      label1.setForeground(Color.white); //468
      label1.setBounds(350,50, size.width, size.height); //469
        ...
        PREVIOUS.addActionListener(this); //493
        
        _header.add(title); //495
        _content.add(label1);
        _content.add(label2);
        _content.add(text1);
        _content.add(text2);
    ...
    public void actionPerformed(ActionEvent ae) 
    {
        ...
        if(ae.getSource() == SUBMIT )
        {
            try
            {
                MyPacker obj = new MyPacker(text1.getText(),text2.getText());
                this.dispose();
                NextPage t = new NextPage("MyAdmin");
            }
            catch(Exception e){}
        }
